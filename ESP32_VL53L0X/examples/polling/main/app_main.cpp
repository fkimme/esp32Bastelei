...
extern "C" void app_main() {
  ...
  if (!vl.setTimingBudget(33000))
    ESP_LOGI(TAG, "Failed to setTimingBudget...");

uint16_t UpperLimitMilliMeter;
if (!vl.getUpperLimitMilliMeter(&UpperLimitMilliMeter))
  ESP_LOGI(TAG, "Failed to getUpperLimitMilliMeter.");

ESP_LOGI(TAG, "UpperLimitMilliMeter: %d [mm]", (int)UpperLimitMilliMeter);
