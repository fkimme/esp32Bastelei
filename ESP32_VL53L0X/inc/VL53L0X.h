...
public:
  ...
  bool init() {
    ...
    VL53L0X_Error status = VL53L0X_SetVcselPulsePeriod(&vl53l0x_dev, VL53L0X_VCSEL_PERIOD_PRE_RANGE, 18);
    if (status != VL53L0X_ERROR_NONE)
      print_pal_error(status, "VL53L0X_VCSEL_PERIOD_PRE_RANGE");

    status = VL53L0X_SetVcselPulsePeriod(&vl53l0x_dev, VL53L0X_VCSEL_PERIOD_FINAL_RANGE, 14);
    if (status != VL53L0X_ERROR_NONE)
      print_pal_error(status, "VL53L0X_VCSEL_PERIOD_FINAL_RANGE");
  ...
  bool getUpperLimitMilliMeter(uint16_t *pUpperLimitMilliMeter)
  {
    // uint16_t pUpperLimitMilliMeter;
    VL53L0X_Error status = VL53L0X_GetUpperLimitMilliMeter(&vl53l0x_dev, pUpperLimitMilliMeter);
    if (status != VL53L0X_ERROR_NONE) {
      print_pal_error(status, "VL53L0X_GetUpperLimitMilliMeter");
      return false;
    }
